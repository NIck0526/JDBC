...
public class Broker implements ActionListener,ItemListener, Runnable{
	...
	*/
		...
		*argument로 받은 CustomerRec[]을 stockList에 하나씩 뿌려준다.<BR>
		...
		public void updateCustomer(){
			...
			try{
				...
				System.out.println("수장힐 고객 엄어여...updateCustomer()");
			}catch(Exception e){
				e.printStackTrace();
			}			
	    } 		
	...
	@Override
	public void run() {
		//10초마다 돌면서 디비에 있는 모든 상장된 주식의 정보를 가져와서
		//stock list에 출력...getAllStocks(), stockList
		while(true) {
			try {
				showStockList(db.getAllStocks(), stockList);
				System.out.println("실시간 주식정보 Reading....");
				Thread.sleep(15000);
			}catch(Exception e) {
